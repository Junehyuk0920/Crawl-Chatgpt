...
def inputQuery(query):
    ...
    # inputDOM = driver.find_element(By.XPATH, '//*[@id="prompt-textarea"]/')
    inputDOM = driver.find_element(By.XPATH, '//textarea')
    inputDOM.send_keys(query)
    inputDOM.send_keys(Keys.ENTER)
    time.sleep(7)

    section = driver.find_elements(By.XPATH, '//div[@class="_wdUoQG_assistantMessage"]')
    currSection = section[-1]
    answers = currSection.find_elements(By.XPATH, './div/div/*')
...
query = sys.argv[1] if len(sys.argv) > 1 else 'No Query.'
inputQuery(query + ' /// 답변 시 인터랙티브 요소 없이, 단순 텍스트 및 리스트 등으로 표현해줘. 즉 마크다운 문법만 사용해서.')
